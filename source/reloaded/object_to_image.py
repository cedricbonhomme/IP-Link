...
def object_to_image(obj_file, image_file):
    ...
    for ip in dic_stat: #draw ip circle
        ...
        for port in port_list:
            ...
            xtmppe, ytmppe = xc+link_radius*math.cos(angtmppe),yc+link_radius*math.sin(angtmppe)
            draw.line((xps,yps,xtmpps,ytmpps), fill="black")
            draw.line((xpe,ype,xtmppe,ytmppe), fill="black")

            (xis, yis, xie, yie) = circular_arc(xc, yc, port_d_radius, last_angle2, last_angle2+angle2i, draw, cur_color2[1])
            ...
            xtmpis, ytmpis = xc+link_radius*math.cos(angtmpis),yc+link_radius*math.sin(angtmpis)
            xtmpie, ytmpie = xc+link_radius*math.cos(angtmpie),yc+link_radius*math.sin(angtmpie)
            xtmpos, ytmpos = xc+link_radius*math.cos(angtmpos),yc+link_radius*math.sin(angtmpos)
            xtmpoe, ytmpoe = xc+link_radius*math.cos(angtmpoe),yc+link_radius*math.sin(angtmpoe)
            ...
            if port_num_in>0:
                rotate_text(im, (int(xt1),int(yt1)), "in", int(round(last_angle2+angle2i/2)), cur_color2[1])
            if port_num_out>0:
                rotate_text(im, (int(xt2),int(yt2)), "out", int(round(last_angle2+angle2/2)), cur_color2[0])
        ...
        xtmpe, ytmpe = xc+link_radius*math.cos(angtmpe),yc+link_radius*math.sin(angtmpe)
        draw.line((xs,ys,xtmps,ytmps), fill="black")
        draw.line((xe,ye,xtmpe,ytmpe), fill="black")

        x, y = xc + ip_radius * math.cos(math.radians(last_angle+angle/2)), xc + ip_radius * math.sin(math.radians(last_angle+angle/2))
        #draw.text((x,y), ip, fill=cur_color[0])
        #print last_angle+angle/2, ip
        rotate_text(im, (int(x),int(y)), ip, int(round(last_angle+angle/2)), cur_color[0])
        last_angle += angle
        ...
        for ip_dst in dic_link[ip_src]:
            for port_src in dic_link[ip_src][ip_dst]:
                for port_dst in dic_link[ip_src][ip_dst][port_src]:
                    ...
                    dic_info["port"][key2]["linki"][key1]["weight"] = dic_link[ip_src][ip_dst][port_src][port_dst]
                    dic_info["port"][key1]["linko"][key2] = {}
                    dic_info["port"][key1]["linko"][key2]["weight"] = dic_link[ip_src][ip_dst][port_src][port_dst]
    ...
    for key in dic_info["port"]:
        ...
        for key2 in dic_info["port"][key]["linki"]:
            ...
            (xs, ys, xe, ye) = circular_arc(xc, yc, link_radius, last_angle, last_angle+angle, draw, cur_color)
            dic_info["port"][key]["linki"][key2]["start"] = (xs, ys, last_angle)
            dic_info["port"][key]["linki"][key2]["end"] = (xe, ye, last_angle+angle)
            x, y = xc + link_radius * math.cos(math.radians(last_angle+angle/2)), xc + link_radius * math.sin(math.radians(last_angle+angle/2))
            #draw.text((x,y), key2, fill=cur_color)
            kip, kport = key2.split(":")
            rotate_text(im, (int(x),int(y)), kport, int(round(last_angle+angle/2)), cur_color)
            last_angle += angle
...
if __name__ == "__main__":
    ...
    parser.add_option("-o", "--output", dest="image_file",
                    help="Image file")
    parser.add_option("-q", "--quiet",
                    action="store_false", dest="verbose",
                    help="be vewwy quiet (I'm hunting wabbits)")
    parser.set_defaults(obj_file = './data/dic.pyobj',
                    image_file = './data/ip.png',
                    verbose = True)
