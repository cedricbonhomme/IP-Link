...
pied = "<h6>IP-Link - Copyright (c) 2009 J&eacute;r&ocirc;me Hussenet, " + \
        "Copyright (c) 2009 C&eacute;dric Bonhomme</h6></body>\n</html>"
html = html_head + "\n"
...
for ip_src in dic_ip:
    ...
    html += "<h2>IP contacted by " + \
            '<a href="./' + ip_src + '.details.html">' + ip_src + "</a></h2>\n\n"
    html += '<div align="center">\n<table border="1" width="50% align="center">\n'
    for ip_dst in dic_ip[ip_src]:
        html += "<tr>\n<td>" + ip_dst + "</td>" + '<td align="center">' + \
                str(dic_ip[ip_src][ip_dst]) + "</td>\n</tr>\n"
    html += "</table>\n</div>\n\n"


    # Details HTMl pages
    html_details = html_head + "\n\n<h2>Report for " + ip_src + "</h2><br />\n\n" + \
                    "<h3>Most contacted IP by " + ip_src + "</h3><br />\n\n"
    html_details += '<div align="center">\n<img src="./images/' + ip_src.replace('.', '') + \
                    '_hist.png" align="bottom" alt="' + ip_src + '" title="' + ip_src + '" />\n</div>'
    html_details += '\n\n<br /><br />\n'
    html_details += '<div align="center">\n<img src="./images/' + ip_src.replace('.', '') + \
                    '_pie.png" align="bottom" alt="' + ip_src + '" title="' + ip_src + '" />\n</div>'
    html_details += '\n\n<h3>Legend :</h3>\n<ul>\n'
    ...
    for i, ip in enumerate(ipdst):
        if ip in dic_ip:
            html_details += '<li>' + str(i+1) + " : " + '<a href="./' + \
                            ip + '.details.html">' + ip +\
                            '</a>' + ", weight : " + str(weight[i]) + '</li>\n'
        else:
            html_details += '<li>' + str(i+1) + " : " + ip + ", weight : " + str(weight[i]) + '</li>\n'
    html_details += '</ul>\n'
    html_details += pied
